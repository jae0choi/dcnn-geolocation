...
if __name__ == "__main__":
    ...
    parser.add_argument("--top-k", action="store_true", help="Use all the predictions in pred file.")
    parser.add_argument("--save", action="store_true", help="Save graph of results.")
    parser.add_argument("--acc", action="store_true", help="Calculate accuracy as well as geolocation error.")
    parser.add_argument("--filter", help="Use only images from this list to analyze results.")
    ...
    for img, preds in predictions.iteritems():
        ...
        #     f_out.write("{}\t{}\n".format(imname, d[0]))
        if args.top_k: top_distances.append(min(d))
        if args.acc:
            total += 1
