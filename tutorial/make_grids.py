...
def open_dataset(filename, lat_idx, lng_idx):
    image_data = []
    with open(filename, 'r') as f:
        for line in f:
            ...
            image_data.append(Image(data, lat_idx, lng_idx))
    return image_data
...
class Image:
    def __init__(self, data, lat_idx, lng_idx):
        self.data = data
        self.lat = float(data[lat_idx])
        self.lng = float(data[lng_idx])
...
def process(train_file, t1, t2, prefix, val_file):
    ...
    '''
    train_file, val_file = args.data_path, args.val_path
    t1, t2 = args.t1, args.t2
    prefix = args.save_prefix
    lat_idx, lng_idx = args.lat_idx, args.lng_idx
    image_data = open_dataset(train_file, lat_idx, lng_idx)
    val_data = None
    if val_file:
        val_data = open_dataset(val_file, lat_idx, lng_idx)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lat-idx', type=int, default=3)
    parser.add_argument('--lng-idx', type=int, default=2)
